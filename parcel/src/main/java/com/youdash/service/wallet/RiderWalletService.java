...
public interface RiderWalletService {
         ...
         */
        void settleOrderDelivered(OrderEntity order, CodCollectionMode codMode, Double codCollectedAmount,
                        Long actorUserId, String actorType);

        /**
         * Re-runs delivery settlement for DELIVERED orders when the delivery rider
         * wallet credit is still missing.
        ...
        /**
         * True when all expected per-rider financial rows exist for this order (one for
         * INCITY / single rider,
        ...
        /**
         * Sum of settled rider earnings for all riders on this order; if none settled
         * yet, full-trip estimate
         * (same as a single rider would see before split).
         */
        double resolveRiderEarningForOrder(OrderEntity order);

        /**
         * This rider's settled earning for the order, or an estimate from
         * first/last-mile share for OUTSTATION split riders.
         */
